 ...
 * @author bazi
 */
public class MainPartExtractor2
{
    ...
    public static MainPart getThirdPart(CoreMap sentence)
    {
        // 去掉不可见字符
        //sentence = sentence.replace("\\s+", "");
        // 分词，用空格隔开
    ...
    public static MainPart getFourthPart(CoreMap sentence)
    {
        // 去掉不可见字符
        //sentence = sentence.replace("\\s+", "");
        // 分词，用空格隔开
    ...
    public static MainPart getFifthPart(CoreMap sentence)
    {
        // 去掉不可见字符
        //sentence = sentence.replace("\\s+", "");
        // 分词，用空格隔开
    ...
    public static MainPart getSixthPart(CoreMap sentence)
    {
        // 去掉不可见字符
        //sentence = sentence.replace("\\s+", "");
        // 分词，用空格隔开
    ...
    public static MainPart getSeventhPart(CoreMap sentence)
    {
        // 去掉不可见字符
        //sentence = sentence.replace("\\s+", "");
        // 分词，用空格隔开
...
//    	mainPart.object.setValue("-");
       ...
       //LOG.info("句法树:{}", tree.pennString());
        ...
        switch (tree.firstChild().label().toString())
        {
            ...
            default:
                ...
                // 尝试合并主语和谓语中的名词性短语
                combineObject2(tdls, mainPart.predicate,mainPart.object);
                //replaceToNN(tdls,mainPart.object,mainPart.object);
                replaceTotraverseNN(tdls,mainPart.object,mainPart.object);
                if (!mainPart.isDone()) mainPart.done();
                mainPart.r2.add(mainPart.subject);
                mainPart.r2.add(mainPart.predicate);
                mainPart.r2.add(mainPart.object);
       ...
       //LOG.info("句法树:{}", tree.pennString());
        ...
        switch (tree.firstChild().label().toString())
        {
            ...
            default:
                ...
                // 尝试合并主语和谓语中的名词性短语
                traverseNN(tdls,mainPart.subject,mainPart.subject);
                if (!mainPart.isDone()) mainPart.done();
                mainPart.r2.add(mainPart.subject);
                mainPart.r2.add(mainPart.predicate);
                mainPart.r2.add(mainPart.object);
    ...
    /*第四句型 */
    public static MainPart getFourthPart(List<Word> words){
    	MainPart mainPart = new MainPart();
        if (words == null || words.size() == 0) return mainPart;
        Tree tree = lp.parse(words);
       //LOG.info("句法树:{}", tree.pennString());
        ...
        switch (tree.firstChild().label().toString())
        {
            ...
            default:
                ...
                // 尝试合并主语和谓语中的名词性短语
                traverseNN(tdls,mainPart.object,mainPart.object);
                if (!mainPart.isDone()) mainPart.done();
                mainPart.r2.add(mainPart.subject);
                mainPart.r2.add(mainPart.predicate);
                mainPart.r2.add(mainPart.object);
    ...
    /*第五句型 */
    public static MainPart getFifthPart(List<Word> words){
    	MainPart mainPart = new MainPart();
        if (words == null || words.size() == 0) return mainPart;
        Tree tree = lp.parse(words);
       //LOG.info("句法树:{}", tree.pennString());
    ...
    /*第六句型 */
    public static MainPart getSixthPart(List<Word> words){
    	MainPart mainPart = new MainPart();
        if (words == null || words.size() == 0) return mainPart;
        Tree tree = lp.parse(words);
       //LOG.info("句法树:{}", tree.pennString());
        ...
        switch (tree.firstChild().label().toString())
        {
            ...
            default:
                ...
                // 尝试合并主语和谓语中的名词性短语
                replaceToCCOMP(tdls,mainPart.predicate);
                if (!mainPart.isDone()) mainPart.done();
                mainPart.r2.add(mainPart.subject);
                mainPart.r2.add(mainPart.predicate);
                mainPart.r2.add(mainPart.object);
    ...
    /*第七句型 */
    public static MainPart getSeventhPart(List<Word> words){
    	MainPart mainPart = new MainPart();
        if (words == null || words.size() == 0) return mainPart;
        Tree tree = lp.parse(words);
       //LOG.info("句法树:{}", tree.pennString());
    ...
    private static List<Word> seg(CoreMap sentence)
    {
        ...
        Collection<TypedDependency> terms2 = graph.typedDependencies();
    ...
    {	
		...
		for(CoreMap sentence: sentences) {
            ...
            mp = MainPartExtractor2.getMainPart(sentence);
        	mp.sentence.setValue(sentence.toString());
        	result2.add(mp.sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
            mp = MainPartExtractor2.getSecondPart(sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
            mp = MainPartExtractor2.getThirdPart(sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
            mp = MainPartExtractor2.getFourthPart(sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
            mp = MainPartExtractor2.getFifthPart(sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
            mp = MainPartExtractor2.getSixthPart(sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
            mp = MainPartExtractor2.getSeventhPart(sentence);
            System.out.printf("%s\t%s\n", sentence, mp);
            result2.addAll(mp.r2);
...
//            }
		...
		OntModel dataModel = ModelFactory.createOntologyModel(OntModelSpec.OWL_DL_MEM);
			for(int i = 1;i<result.get(k).size();i=i+3){
			indiv1 = result.get(k).get(i).value(); //主语
			indiv2 = result.get(k).get(i+2).value(); //宾语	
			annotation = result.get(k).get(i+1).value(); //谓语
...
//			}
			...
			if (validity.isValid()) {
			    ...
			    System.out.println(result.get(k).get(0).value());
			    for (Iterator i = validity.getReports(); i.hasNext(); ) {
			        ValidityReport.Report report = (ValidityReport.Report)i.next();
			        System.out.println(" - " + report);
			    }
			}
			immodel.close();//资源占用太多，手动关闭已开资源
			ismodel.close();
